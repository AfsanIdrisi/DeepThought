    ...
    "tasks": [
     {
        ...
        "assets": [
          ...
          },
          {
            ...
            "asset_description": "Write a 400-500 word article, from your thread. and showcase your learning to the entire world.",
            "asset_content": " ",
            "asset_type": "input_asset",
...
let journey_task_title=document.getElementById("journey_task_title");
let task_description=document.getElementById("task_description");
let asset_title=document.getElementsByClassName("asset_title");
let journey_asset_title=document.getElementsByClassName("journey_asset_title");
let asset_description=document.getElementsByClassName("asset_description");
let asset_source=document.getElementById("asset_source");
//Got Elements From DOM

// Changing Data From Json 
title.innerHTML=data.title;
task_title.innerHTML=data.tasks[0].task_title;
journey_task_title.innerHTML=data.tasks[0].task_title;
task_description.innerHTML=data.tasks[0].task_description;
 
for(let i=0;i<data.tasks[0].assets.length;i++){
    asset_title[i].innerHTML=data.tasks[0].assets[i].asset_title;
    asset_description[i].innerHTML=data.tasks[0].assets[i].asset_description;
  }
  asset_source.src=data.tasks[0].assets[0].asset_content;
  for(let i=0;i<data.tasks[0].assets.length;i++){
    journey_asset_title[i].innerHTML=data.tasks[0].assets[i].asset_title;
    }